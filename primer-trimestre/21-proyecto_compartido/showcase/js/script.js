...
RESET_BOTON.addEventListener("click", resetearBotones);
...
function mostrarNotas(nombreAlumno) {
  ...
  if (ALUMNO) {
    ...
    borrarListaNotas(NOTAS_LISTA_ELEMENTO);
...
function mostrarPromedio() {
  const NOMBRE_ALUMNO = obtenerNombreAlumno();
...
function mostrarAlgunSuspenso() {
  const NOMBRE_ALUMNO = obtenerNombreAlumno();
  mostrarEstadistica(NOMBRE_ALUMNO, buscarAlgunSupenso, ALGUN_SUSPENSO_TEXTO_ELEMENTO);
}

function buscarAlgunSupenso(notas) {
...
function borrarListaNotas(elemento) {
